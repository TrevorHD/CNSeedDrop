...
anova(mod.warm, lm(TV~MowYN + Warming, data = data.new), mod.intYN)

# no interaction (E/L mow)
mod.mowWarm = lm(TV ~ Mow + Warming, data = data.new)
summary(mod.mowWarm)
mowWarm.dat = expand.grid(Mow = c("CTL", "E", "L"), Warming = c("A", "W"))
mowWarm.dat = cbind(mowWarm.dat, predict(mod.mowWarm, newdata = mowWarm.dat, interval = "confidence"))
...
ggplot(data = mowWarm.dat, aes(x = Mow, color = Warming))+
  ...
  labs(y = "Estimated Terminal Velocity")+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
anova(mod.warm, mod.mowWarm, mod.int)
```

```{r TV_ByTreatment_ANOVAInt_log}
# warming
...
# both warming and mowing
mod.mowWarm = lm(SeedWidth ~ Mow + Warming, data= data.new)
summary(mod.mowWarm)

SeedWidth.dat = expand.grid(Mow = c("CTL", "E", "L"), Warming = c("A", "W"))
SeedWidth.dat = cbind(SeedWidth.dat, predict(mod.mowWarm, newdata = SeedWidth.dat, interval = "confidence"))
SeedWidth.dat = do.call(data.frame, SeedWidth.dat)

ggplot(data = SeedWidth.dat, aes(x = Mow, color = Warming))+
  geom_point(aes(y = fit), position = position_dodge(0.5), size = 2, shape = 15)+
  geom_errorbar(aes(ymin = lwr, ymax = upr), width = 0.2, position = position_dodge(0.5))+
  labs(y = "Estimated Seed Width")+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
# partial F test: compare full model to warming only model
anova(mod.mow, mod.mowWarm, mod.int)


# both warming and mowing YN
mod.mowWarmYN = lm(SeedWidth ~ MowYN + Warming, data= data.new)
summary(mod.mowWarmYN)
...
SeedWidth.dat = expand.grid(MowYN = c(0,1), Warming = c("A", "W"))
SeedWidth.dat$Mowing = ifelse(SeedWidth.dat$MowYN == 1, "Y", "N")
SeedWidth.dat = cbind(SeedWidth.dat, predict(mod.intYN, newdata = SeedWidth.dat, interval = "confidence"))
SeedWidth.dat = do.call(data.frame, SeedWidth.dat)

ggplot(data = SeedWidth.dat, aes(x = Mowing, color = Warming))+
  geom_point(aes(y = fit), position = position_dodge(0.5), size = 2, shape = 15)+
  geom_errorbar(aes(ymin = lwr, ymax = upr), width = 0.2, position = position_dodge(0.5))+
  labs(y = "Estimated Seed Width")+
  theme_bw()+
...
bar.group(out$groups, ylim = c(0,25), xlab = "MowYN", ylab = "Seed Width", main = "Tukey HSD")
# partial F test: compare full model to warming only model
anova(mod.mowYN, lm(SeedWidth~MowYN + Warming, data = data.new), mod.intYN)

stepAIC(mod.int, direction = "both")
stepAIC(mod.int, direction = "backward")
