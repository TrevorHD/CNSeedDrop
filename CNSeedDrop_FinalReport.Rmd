...
Backward and bi-directional selection for AIC and BIC support the warmign and mowing model, though forward selection suggests an intercept only model in both cases of AIC and BIC. The AIC and BIC for the warming and mowing model is ```r round(AIC(fitTV.select$AICBoth),2) ``` and ```r round(AIC(fitTV.select$AICBoth, k = log(length(data.new))),2) ```, respectively, whereas the intercept only model has ```r round(AIC(fitTV.select$AICForward),2) ``` and ```r round(AIC(fitTV.select$AICForward, k = log(length(data.new))),2) ``` respectively.  

Thus, based on the results of the partial F-test and AIC/BIC model selection, we conclude that $Terminal Velocity = \beta_0 + \beta_1 Mowing+ \beta_2  Warming$ is the best model for our data. The residuals for this model are shown in Figure \ref{fig:TV_Resids}. The residuals show approximately random distribution, and though there are a few points with high leverage, they do are not high includence accroding to Cook's distance. Lastly, we can see that our log-transformation significantly improved the normality of the errors, as shown in the Normal Q-Q plot. 
...
Again, we use AIC/BIC model selection to validate these results.

```{r SW_ModelSelection, include = FALSE}
fit.SW = lm(SeedWidth ~ Warming * Mow, data = data.new)
fitSW.select = list()
#AIC
fitSW.select$AICBoth = stepAIC(fit.SW, direction = "both")
fitSW.select$AICBackward = stepAIC(fit.SW, direction =  "backward")
fitSW.select$AICForward = stepAIC(lm(SeedWidth~1, data = data.new), direction =  "forward")
#BIC
fitSW.select$BICBoth = stepAIC(fit.SW, direction = "both", k = log(length(data.new)))
fitSW.select$BICBackward = stepAIC(fit.SW, direction =  "backward", k = log(length(data.new)))
fitSW.select$BICForward = stepAIC(lm(SeedWidth~1, data = data.new), direction =  "forward", k = log(length(data.new)))
```

```{r SW_ModelSelectionTable, fig.cap="\\label{fig:TV_ModelSelectionTable}CAPTION"}
# results of model selection
SW.selection = expand.grid(Variable = c("(Intercept)","MowE","WarmingW", "WarmingW:MowE"), Method = c("AICForward", "AICBackward", "AICBoth", "BICForward", "BICBackward", "BICBoth"))
SW.selection$Included = NA


for(i in SW.selection$Method){
  for(j in SW.selection$Variable){
    SW.selection$Included[SW.selection$Method == i & SW.selection$Variable == j] = ifelse( j %in% names(fitSW.select[[i]][["coefficients"]]), 1,0 )
  }
}

SW.selection$id <- rep(1:4,times = 6)
SW.selection = dcast(data = SW.selection,formula = Variable~Method,fun.aggregate = sum,value.var = "Included")
SW.selection[,1] = c("Intercept","Mowing","Warming","Warming:Mowing")
colnames(SW.selection) = c("Variable", "AIC: Forward", "AIC: Backward", "AIC: Both", "BIC: Forward", "BIC: Backward", "BIC: Both")

kable(SW.selection)
```

Again, we see similar results to those in Research Question 2. The results of the AIC/BIC  backward and bi-directional suggest the warming and mowing model is the best fit (AIC: ```r round(AIC(fitSW.select$AICBoth),2) ``` and BIC: ```r round(AIC(fitSW.select$AICBoth, k = log(length(data.new))),2) ```,), wherease forward selection indicates an intercept only model (AIC:```r round(AIC(fitSW.select$AICForward),2) ``` and  BIC:```r round(AIC(fitSW.select$AICForward, k = log(length(data.new))),2) ```).  Based on the AIC and BIC values of the warminga nd mowing vs. intercept only model, we conclude that the warming and mowing model is the best fit. 
...
To investigate the pairwise comparisons of seed width between each group, we performed a post-hoc Tukey HSD test. 

```{r SW_Tukey, fig.width = 6, fig.cap="\\label{fig:SW_Tukey}Results of post-hoc Tukey HSD test. Letters indicate groups which are significantly different. "}
