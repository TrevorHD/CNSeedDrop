...
The results from the model selection can be seen below, along with the VIF for each term and AIC for each resulting model. All coefficients are significant to $\alpha < 0.001$.
...
par(mfrow = c(2, 1))
plot(mod.3, which = 2, cex = 2)
plot(mod.4, which = 2, cex = 2)
```

``` {r, fig.height = 14, fig.width = 12}
par(mfrow = c(2, 1))
plot(mod.3, which = 1, cex = 2)
plot(mod.4, which = 1, cex = 2)
```
