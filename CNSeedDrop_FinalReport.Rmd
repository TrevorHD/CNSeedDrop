...
The model resulting from this analysis suggests two things. First, pappus width is the most important aspect of pappus shape in determining seed terminal velocity; while other aspects of the pappus shape may be used to predict seed terminal velocity, they are not particularly helpful when pappus width is already considered. Second, seed terminal velocity is negatively correlated with pappus width: terminal velocity decreases as pappus width increases, and vice-versa.
...
Because we are fitting a continutous response variable to categorical predictor variables, we can use an ANOVA model to characterize changes in terminal velocity by treatment. In order to compensate for the skewed right distribution of the terminal velocity data and significant deviations from error normality (Figure \ref{fig:TVHistAndErrors}), we log-transform terminal velocity for this analysis. 

```{r TVHistAndErrors,fig.height = 3 ,fig.cap = "\\label{fig:TVHistAndErrors} Histogram of terminal velocity (A) and Normal Q-Q plot of linear model with both warming and mowing predictors (B)."}
...
We use two methods to identify the most appropriate model fit for the log-transformed terminal velocity data, starting by fitting the following four models:
