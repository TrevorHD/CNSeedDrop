...
# Abstract
Main points: 
RQ2 - early mowing increased TV whereas warming decreased TV. There was no significnat effect observed for late mowed plants compared to control plants
...
To validate these model selection results, we also evaluated both the AIC and BIC for each model, and results are shown in the table below. 

```{r TV_AIC}
AIC = data.frame(Model =  c("Mowing","Warming", "Warming and Mowing", "Interaction"), AIC = c(AIC(mod.mow.log), AIC(mod.mow.log), AIC(mod.mowWarm.log), AIC(mod.int.log)), BIC = c(AIC(mod.mow.log, k = log(length(data.new))), AIC(mod.mow.log,k = log(length(data.new))), AIC(mod.mowWarm.log, k = log(length(data.new))), AIC(mod.int.log,k = log(length(data.new)))))
AIC[2,3] = round(AIC[2,3],2)

kable(AIC)
```

Both AIC and BIC support the warmign and mowing model,  and thus, based on the results of the partial F-test and AIC/BIC model selection, we conclude that $Terminal Velocity = \beta_0 + \beta_1 Mowing+ \beta_2  Warming$ is the best model for our terminal velocity data. The residuals for this model are shown in Figure \ref{fig:TV_Resids}. The residuals show approximately random distribution, and though there are a few points with high leverage, they do are not high includence accroding to Cook's distance. Lastly, we can see that our log-transformation significantly improved the normality of the errors, as shown in the Normal Q-Q plot. 
...
Model results are shown in the table below and treatment level predictions are shown in Figure \ref{fig:TV_ANOVATrtEst}. From these results, we conclude that early mowing significnatly decreases terminal velocity, whereas warming increases terminal velocity. 

```{r TV_ModelResults}
kable(round(summary(mod.mowWarm.log)$coefficients,4))
```
...
# Research Question 3
Next, we will explore the extent to which the results of Research Question 2, or the changes in terminal velocity by treatment, can be explained by seed width, as the results of Research Question 1 suggest that seed width is a good predictor of terminal velocity. Figure \ref{fig:SWbarplot} shows the variation in seed width across treatments. 
...
barplot.dat = aggregate(data.new$SeedWidth, by = list(Mowing = data.new$Mow, Warming = data.new$Warming), FUN = function(x) c(mean(x),sd = sd(x), n = length(x)))
barplot.dat = do.call(data.frame, barplot.dat)
barplot.dat$SE = barplot.dat$x.sd / sqrt(barplot.dat$x.n)
colnames(barplot.dat) = c("Mowing", "Warming", "Mean", "SD", "n", "SE")
...
ggplot(data = barplot.dat, aes(Mowing, Mean, fill = Warming))+
  geom_bar(stat = "identity", position = position_dodge(0.9))+
  geom_point(data = data.new, aes(x = Mow, group = Warming, y = SeedWidth), position = position_jitterdodge(dodge.width = 0.9), alpha = 0.5, shape = 16)+
  geom_errorbar(aes(ymin = Mean - SE, ymax = Mean + SE), width = 0.2, position = position_dodge(0.9))+
  labs(y = "Seed Width")+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
Here, the warming only model has a p-value of ```r round( pf(F.warm[1],F.warm[2],F.warm[3],lower.tail=F),4)``` whereas the mowing only model has a p-value of ```r  round(pf(F.mow[1],F.mow[2],F.mow[3],lower.tail=F),4)```. Therefore, we use the mowing only model as the base model in our partial F-test. Results, shown in the table below, suggest that the model with both mowing and warming, but no interaction term is again the best fit for the data. 
...
Again, we use AIC and BIC model selection criteria to validate these results.

```{r SW_AIC}
AIC = data.frame(Model =  c("Mowing","Warming", "Warming and Mowing", "Interaction"), AIC = c(AIC(SW.mow), AIC(SW.warm), AIC(SW.mowWarm), AIC(SW.int)), BIC = c(AIC(SW.mow, k = log(length(data.new))), AIC(SW.warm,k = log(length(data.new))), AIC(SW.mowWarm, k = log(length(data.new))), AIC(SW.int,k = log(length(data.new)))))
AIC[2,3] = round(AIC[2,3],2)

kable(AIC)
```

We see similar results to those in Research Question 2, suggesting that the warming and mowing model is the best fit. Thus, both the partial F-test and the AIC/BIC model selection support the mowing and warming model, whose model diagnostic plots are shown in Figure \ref{fig:SW_Resids}. These residuals suggest our errors are approximately normally distributed. Again, there may be some leverage points, though they do not have high influence. 

```{r SW_Resids, fig.height=8, fig.cap="\\label{fig:SW_Resids}Diagnostic plots for predicting seed width using warming and mowing model."}
par(mfrow = c(2,2))
plot(SW.mowWarm)
```

The table below shows the results of the mowing and warming model for predicting seed width. These results suggest that both early mowing, late mowing, and warming signficantly decrease seed width. 

```{r SW_ModelResults}
kable(round(summary(SW.mowWarm)$coefficients,4))
```

To investigate the pairwise comparisons of seed width between each group, we performed a post-hoc Tukey HSD test (Figure \ref{fig:SW_Tukey}). 

```{r SW_Tukey, fig.width = 5.8, fig.cap="\\label{fig:SW_Tukey}Results of post-hoc Tukey HSD test. Letters indicate groups which are significantly different, where $\alpha = 0.05$ is the significance level. "}
...
```{r SWbin_AIC}
AIC = data.frame(Model =  c("Mowing","Warming", "Warming and Mowing", "Interaction"), AIC = c(AIC(SWbin.mow), AIC(SWbin.warm), AIC(SWbin.mowWarm), AIC(SWbin.int)), BIC = c(AIC(SWbin.mow, k = log(length(data.new))), AIC(SWbin.warm,k = log(length(data.new))), AIC(SWbin.mowWarm, k = log(length(data.new))), AIC(SWbin.int,k = log(length(data.new)))))
AIC[2,3] = round(AIC[2,3],2)
kable(AIC)
```

Discerning between the mowing and warming vs. the interaction models is less clear in this case. On one hand, the partial F-test suggests the interaction model may provide a slightly better fit and the interaction model has the lowest AIC value, but the warming and mowing model has the lowest BIC value. This result is not unexpected, as a slight improvement in model fit may not be enough to overcome the BIC penality for an additional term in the interaction model. 

The results of both models are summarized in Table XX. In the warming and mowing model, both treatments decrease seed width, whereas when an interaction term is added, neither of these terms are significant, but the interaction term itself becomes significant at a level of 0.1.  
...
kable(df, caption = "Results of Mowing and Warming and Interaction models, respectively.")
```

These two results suggest that plants which are both warmed and mowed have a significantly smaller seed width. These plants are likely driving the significant mowing and warming coefficients in the model without interaction. Another post-hoc Tukey test, shown in Figure \ref{fig:SWbin_Tukey}, supports this conclusion. 

```{r SWbin_Tukey, fig.width = 5.8, fig.cap="\\label{fig:SWbin_Tukey}Results of post-hoc Tukey HSD test. Letters indicate groups which are significantly different, where $\alpha = 0.05$ is the significance level. "}
...
The results of this experiment contributes to our understanding of how *Carduus nutans* seeds may disperse under warmer conditions, and further how mowing management may mediate these changes. We found that, as is predicted by physical models$^4$, that the width of the seed is a good predictor of terminal velcoity. Smaller seeds fall faster, and thus disperse less far. 

We also found that both mowing and warming treatments significantly affected seed terminal velocity. Early mowing decreased terminal velocity, while warming increased terminal velocity. Thus, all else being equal, we expect mowed plants to disperse further and warmed plants to disperse less far. 
