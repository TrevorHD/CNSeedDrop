...
```{r SW_Resids, fig.height = 14, fig.width = 12, fig.cap="\\label{fig:SW_Resids}Diagnostic plots for predicting seed width using `Warming and Mowing` model."}
par(mfrow = c(2,2))
plot(SW.mowWarm)
```
...
The results of both models are summarized in Table \ref{tab:SWbin_ModelResults}. In the warming and mowing model, both treatments decrease pappus width; whereas when an interaction term is added, neither the warming nor mowing terms are significant, but the interaction term itself becomes significant at a significance level of 0.1.  
